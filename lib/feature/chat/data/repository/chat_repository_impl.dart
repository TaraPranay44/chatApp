...
class ChatRepositoryImpl implements ChatRepository {
  ...
  Future<Either<Failure, ChatEntity>> findOrCreateChat({
    ...
    try {
      ...
      if (user1Id.isEmpty || user2Id.isEmpty) {
        return const Left(
            ValidationFailure(message: 'User IDs cannot be empty'));
      }
      ...
      final chatResponse =
          await _remoteDataSource.findOrCreateChat(request: request);
  ...
  Future<Either<Failure, List<MessageEntity>>> getChatMessages({
    ...
    try {
      ...
      // Convert to entities
      final messageEntities = messageResponse.messages
              ?.map((message) => _convertMessageModelToEntity(message))
              .toList() ??
          [];
  ...
  Future<Either<Failure, MessageEntity>> sendMessage({
    ...
    try {
      ...
      final sendMessageResponse =
          await _remoteDataSource.sendMessage(request: request);

      // Convert to entity
      final messageEntity =
          _convertMessageModelToEntity(sendMessageResponse.message);
  ...
  MessageEntity _convertMessageModelToEntity(dynamic message, {int chadId = 0}) {
    if (message == null) {
